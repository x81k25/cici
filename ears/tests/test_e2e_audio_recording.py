...
class TestDebugModeNoFalsePositives:
    """Test that normal audio files don't trigger unexpected defects."""

    # Minor/expected defects in normal speech recordings
    # These are normal characteristics, not problems
    EXPECTED_MINOR_DEFECTS = {
        "silence",           # Leading/trailing silence, pauses between words
        "low_volume",        # Quiet transitions between speech
        "noise_only",        # Breath sounds, ambient noise between words
        "wrong_chunk_size",  # Edge chunks at file boundaries
    }

    # Serious defects that indicate actual audio problems
    SERIOUS_DEFECTS = {
        "clipping",
        "dc_offset",
        "wrong_byte_order",
    ...
    def test_normal_audio_no_serious_defects(self, transcription_test_cases: list[dict]):
        """Test that valid audio files don't produce serious defects."""
        from ears.audio.audio_analyzer import AudioAnalyzer

        analyzer = AudioAnalyzer()
        files_with_serious_defects = {}
        files_with_minor_defects = {}
        ...
        for case in transcription_test_cases:
            ...
            for i, chunk in enumerate(chunks):
                metrics, defects = analyzer.analyze_chunk(chunk)
                for d in defects:
                    entry = f"chunk {i}: {d.code} - {d.message}"
                    if d.code in self.SERIOUS_DEFECTS:
                        serious_defects.append(entry)
                    else:
                        minor_defects.append(entry)

            # Check stream-level defects
            final_defects = analyzer.finalize()
            for d in final_defects:
                entry = f"finalize: {d.code} - {d.message}"
                if d.code in self.SERIOUS_DEFECTS:
                    serious_defects.append(entry)
                else:
                    minor_defects.append(entry)
            ...
            if serious_defects:
                files_with_serious_defects[case["filename"]] = serious_defects
            if minor_defects:
                files_with_minor_defects[case["filename"]] = minor_defects

            analyzer.reset()

        # Fail only on serious defects
        if files_with_serious_defects:
            report = "\n".join([
                f"\n{filename}:\n  " + "\n  ".join(defects)
                for filename, defects in files_with_serious_defects.items()
            ])
            pytest.fail(f"Serious defects detected in normal audio:{report}")

    def test_normal_audio_has_expected_minor_defects(self, transcription_test_cases: list[dict]):
        """Test that normal audio has expected minor defects (silence, low_volume, etc.)."""
        from ears.audio.audio_analyzer import AudioAnalyzer

        analyzer = AudioAnalyzer()
        all_minor_defect_codes = set()
        ...
        for case in transcription_test_cases:
            ...
            for chunk in chunks:
                _, defects = analyzer.analyze_chunk(chunk)
                for d in defects:
                    if d.code in self.EXPECTED_MINOR_DEFECTS:
                        all_minor_defect_codes.add(d.code)

            analyzer.reset()

        # Verify we see the expected minor defects (silence and low_volume at minimum)
        # This confirms our test audio has natural speech characteristics
        assert "silence" in all_minor_defect_codes, \
            "Expected to find silence chunks in normal recordings"
        assert "low_volume" in all_minor_defect_codes, \
            "Expected to find low volume transitions in normal recordings"
